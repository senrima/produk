// ===============================================================
// == JAVASCRIPT LENGKAP UNTUK HALAMAN VIEW PRODUK DENGAN CACHING ==
// ===============================================================
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchAndCacheProducts(forceRefresh = false) {
        ...
        try {
            ...
            if (response.ok && result.status === 'sukses') {
                ...
                // 1. Simpan semua produk untuk pencarian
                sessionStorage.setItem('allProducts', JSON.stringify(products));

                // 2. Kelompokkan produk per kategori untuk filter cepat
    ...
    function renderProducts(products) {
        ...
        // Re-inisialisasi event listener untuk semua elemen interaktif yang baru dibuat
        reinitializeEventListeners();
    }

    /**
     * Membuat HTML untuk satu kartu produk.
     * Kode ini diambil dari file asli Anda.
     */
    ...
    /**
     * Memasang ulang event listener untuk elemen dinamis seperti modal video
     */
    function reinitializeEventListeners() {
        ...
        if (!modalOverlay) return; // Keluar jika elemen modal tidak ada
        ...
        // Pasang listener baru ke tombol yang baru di-clone
        ...
        if (modalOverlay) modalOverlay.onclick = (event) => {
    ...
    filterButtons.forEach(button => {
        button.addEventListener('click', () => {
            ...
            searchInput.value = ''; // Reset pencarian saat filter diklik

            if (filter === 'all') {
                renderProducts(allProducts);
            } else if (categorizedProducts[filter]) {
                renderProducts(categorizedProducts[filter]);
            } else {
                renderProducts([]); // Tampilkan kosong jika kategori tidak ada di cache
            }
        });
    });
    ...
    searchInput.addEventListener('input', () => {
        ...
        searchTimeout = setTimeout(() => {
            // Selalu cari dari parameter "semua produk"
            const allProducts = JSON.parse(sessionStorage.getItem('allProducts')) || [];
            const query = searchInput.value.toLowerCase().trim();
            
            // Atur filter ke "Semua" saat mencari
            filterButtons.forEach(btn => btn.classList.remove('active'));
    ...
    refreshBtn.addEventListener('click', () => {
        ...
        // Hapus cache dan ambil data baru
        ...
        fetchAndCacheProducts(true).finally(() => {
            ...
            // Reset filter ke "Semua" dan kosongkan pencarian
            filterButtons.forEach(btn => btn.classList.remove('active'));
            document.querySelector('.filter-btn[data-filter="all"]').classList.add('active');
            searchInput.value = '';
        });
    ...
    // -- INISIALISASI
    // ---------------------------------------------------------------
    fetchAndCacheProducts(); // Panggil fungsi utama saat halaman dimuat
    updateCartIcon(); // Fungsi ini dari keranjang.js
});
